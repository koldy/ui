...
const ThemeManager = function({json, mode, onDebug, onInfo, onWarning, onError}) {
	this.mode = [DEBUG, STRICT, PRODUCTION].indexOf(mode) >= 0 ? mode : null;
	this.on = {onDebug, onInfo, onWarning, onError};

	this.jsonData = json; // remember the original theme
	...
	this.availableColorsData = Object.keys(json.color || {});

	Object.entries(this.availableColorsData).forEach(([i, key]) => {
		this.colorsData[key] = new Color({
			colors: json.color[key],
			theme: this
		});

		if (i === 0) {
			this.firstColorName = key;
		}
	});
	...
	this.colors = function() {
		return Object.values(this.colorsData);
	};
	...
	this.color = function(colorName) {
		return typeof colorName === 'string' && this.colorsData[colorName] instanceof Color ? this.colorsData[colorName] : null;
	};
	...
	this.firstColor = function() {
		return this.color(this.firstColorName);
	};
			...
			if (colorValue.indexOf(' ') === -1) {
				...
				if (color instanceof Color) {
					...
					if (Number.isNaN(tone)) {
						this.error(`Can not parse color tone from color ${colorValue}; parsing tone into integer resulted in NaN`);
					} else {
